...
def train():
    ...
    print(f"🚀 Preparing ResNet50 training pipeline (Data: {DATA_PATH})...")

    # --- CONFIGURATION ---
    IMG_SIZE = 224  # Using 224 for optimal ResNet50 performance
    BATCH_SIZE = 16
    EPOCHS = 50
    ...
    if os.path.exists(MODEL_FILE):
        ...
        # Ensure all BatchNormalization layers inside base model remain in inference mode
        for layer in model.layers:
            if layer.name == 'resnet50':
                layer.trainable = True
                for sub_layer in layer.layers:
                    if isinstance(sub_layer, BatchNormalization):
        ...
        print("✨ Resuming training session... Aiming for 95%+ accuracy!")
        model.fit(
            train_ds, 
        ...
        print("🆕 No existing model found. Building new ResNet50 model...")
        base_model = ResNet50(weights='imagenet', include_top=False, input_shape=(IMG_SIZE, IMG_SIZE, 3))
        
        # Freeze entire base model first for Stage 1 (Feature Extraction)
        ...
        # Stage 1 Training: Feature Extraction (Warm-up top classification head)
        print("🔥 Stage 1: Training newly initialized classification head (Warm-up)...")
        model.compile(
        ...
        print("\n🔥 Stage 2: Unfreezing top 30 layers of ResNet50 base and fine-tuning...")
        ...
        model.compile(
            optimizer=Adam(learning_rate=1e-5),  # Low learning rate for fine-tuning
            loss='binary_crossentropy',
            metrics=['accuracy']
        )
        ...
        print("✨ Stage 2 Training in progress... Fine-tuning to 95%+ accuracy!")
        model.fit(
            train_ds, 
            validation_data=val_ds, 
